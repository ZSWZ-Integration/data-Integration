...
public class StatisticDao implements StatisticRepository {
    ...
    public List<StatisticCourseVO> getAllCourseStatistic() {
        try{
            List<StatisticCourseVO> statisticCourseVOS=jdbcTemplate.query("select c.cno,c.cname,c.teacher,c.credit,(CASE WHEN count IS NOT NULL THEN count ELSE 0 END)as count from course c left join (select cno,count(*) as count from take_course group by cno)t on c.cno=t.cno;",new StatisticCourseVOMapper());
